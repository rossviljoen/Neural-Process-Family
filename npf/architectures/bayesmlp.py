...
class BayesianLinear(nn.Module):
    def __init__(self, input_size, output_size, prior_mean=0., prior_std=0.05):
        ...
        # self.p_mu = torch.full((output_size, input_size), self.prior_mean, requires_grad=False, device=self.device)
        # self.p_log_sigma = torch.full((output_size, input_size), self.prior_log_sigma, requires_grad=False, device=self.device)
